...
def rsa(d1, d2):
    ...
    outrsa = (power((mean(d1) - mean(d2)), 3)
              ...
              / var(concatenate([d1, d2], 1),
                    ddof=1)  # Delta Degrees of Freedom: the divisor used in the calculation is N - ddof, where N represents the number of elements. By default, ddof is zero.
...
def get_correlation(px, py):
    ...
    # Isolate HFB and LFB
    px_temp = np.divide(px, np.dstack([
                                          px_mean] * num_trials))  # normalize data:every datapoint is divided by the average power of the same freq at the same channel
...
def featurevectors_by_trial(px_base, py_base, trial_types, do_correlation_analysis=False, correlation_pairs=None,
                            ranges=None, corr_threshold=1):
    ...
    It calculates the mean power over the given frequency ranges. It can also ignore channels with p-value over 'corr_treshold' for the given trial type,
