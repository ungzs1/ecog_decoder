...
class StanfordPreprocessor(Preprocessor):
    ...
    Preprocessor.time_range = (0, -1)  # set time range of trials to use in a tuple of (first data, last data),
    # eg (1000,-500) ignores first 1000 and last 500 datapoints. Default: time_range=(0,-1) to use whole range
    Preprocessor.freq_range = (0, 200)  # range of Power Spectrum, min and max freq in a tuple eg.(0,200) gives
    # power spectrum from 0 to 199 Hz. Default: PSD_freq_range=(0,200). NOTE_: max_freq not included!
    Preprocessor.subject_ids = ['bp', 'ca', 'cc', 'de', 'fp', 'gc', 'hh', 'hl', 'jc', 'jm', 'jt', 'rh', 'rr', 'ug',
                                'wc', 'zt']
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ecog_lp = path_to_data  # data load path
    ...
    def get_train_test_data(self, subject):
        ...
        filename = os.path.join(self.ecog_lp, subject+'_mot_t_h.mat')
